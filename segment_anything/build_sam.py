...
from .modeling import ImageEncoderViT, MaskDecoder, PromptEncoder, Sam, TwoWayTransformer,Cls_layer
...
def _build_sam(
    ...
    sam = Sam(
        ...
        cls_layer=Cls_layer(encoder_output_features=prompt_embed_dim, num_classes=5), # num_classes与数据集的类别数对应
    ...
    if checkpoint is not None:
        ...
        pretrained_dict = {k: v for k, v in state_dict.items() if 'mask_decoder' not in k}
        model_dict.update(pretrained_dict)
        try:
            sam.load_state_dict(model_dict,False)
        except:
            print('*******interpolate')
