...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens,image_tasks)
        iou_token_out = hs[:, 0, :]
...
class Cls_layer(nn.Module):
    def __init__(self,encoder_output_features, num_classes=2):

        super(Cls_layer, self).__init__()
        self.pool = nn.AdaptiveAvgPool2d((1, 1))
        self.classifier = nn.Linear(encoder_output_features, num_classes)
