...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        #self.attn = DilatedAttention(head_dim, num_heads, dilation_rates=[1, 2, 4], segment_length=4)
        '''
        dilation_rates = [1, 2, 4, 6, 12]
        segment_lengths = [ 1,2,4,6,12]
        op=None
        dropout = 0.0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv_1 = self.qkv(x).reshape(B, H * W, 3, self.num_heads, -1).half()
        attn_1 = flash_attn_qkvpacked_func(qkv_1)
        x = attn_1.reshape(B,H,W,-1).float()

        '''
        qkv_1 = self.qkv(x).reshape(B, H * W, 3, self.num_heads, -1)
        attn_1 = flash_attn_qkvpacked_func(qkv_1)
        x = attn_1.reshape(B,H,W,-1)
        '''
        #end_time = time.time()
        #print(end_time - start_time)
        '''
        #dailteed attention
        qkv = self.qkv(x).reshape(B, H * W, 3, self.num_heads, -1).permute(2, 0, 1, 3, 4)
        #q, k, v = qkv.reshape(3, B * self.num_heads, H * W, -1).unbind(0)
        q,k,v = qkv.unbind(0)
        x = self.attention(q, k, v, is_causal=None)
        x = rearrange(x, "b (n w) h d -> b n w (h d)",n=H,w=W)
        '''
        x = self.proj(x)
