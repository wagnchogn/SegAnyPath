...
from DataLoader import TrainingDataset, stack_dict_batched,TestingDataset
...
def parse_args():
    ...
    parser.add_argument("--work_dir", type=str,
                        default="",
                        help="work dir")
    parser.add_argument("--run_name", type=str, default="seganypath", help="run model name")
    parser.add_argument("--epochs", type=int, default=15, help="number of epochs")
    parser.add_argument("--batch_size", type=int, default=4, help="train batch size")
    parser.add_argument("--image_size", type=int, default=1024, help="image_size")
    parser.add_argument("--mask_num", type=int, default=3, help="get mask number")
    ...
    parser.add_argument("--point_list", type=list, default=[1,  5, 9,12], help="point_list")
    parser.add_argument("--multimask", type=bool, default=True, help="ouput multimask")
    parser.add_argument("--encoder_adapter", type=bool, default=True, help="use adapter")
    parser.add_argument("--use_amp", type=bool, default=False, help="use amp")
    parser.add_argument("--use_pretrain", type=bool, default=False, help="use tcga")
    parser.add_argument("--pretrain_pth", type=str,
                        default="",
                        help="pretrian checkpoint")
...
def prompt_and_decoder(args, batched_input, model, image_embeddings, decoder_iter=False,image_tasks=None):
    ...
    low_res_masks, iou_predictions = model.mask_decoder(
        ...
        multimask_output=args.val_multimask,
        image_tasks = image_tasks,
    )

    if args.val_multimask:
...
def train_one_epoch(args, model, optimizer, train_loader, epoch, criterion,cls_criterion):
    model.train()
    ...
    for batch, batched_input in enumerate(train_loader):
        ...
        if args.use_amp:
            ...
            masks, low_res_masks, iou_predictions = prompt_and_decoder(args, batched_input, model, image_embeddings,
                                                                       decoder_iter=False,image_tasks=image_tasks_repeat)
            ...
            mag_label = batched_input["mag_label"]

            pred_mag_label = model.cls_layer(image_embeddings)
            mag_loss = cls_criterion(pred_mag_label, mag_label)
            ...
            for i in range(batch):
                ...
                image_embed = image_embed.repeat(args.mask_num, 1, 1, 1)
                image_embeddings_repeat.append(image_embed)
                image_task = batched_input["task_label"][i].repeat(args.mask_num, 1)
                image_tasks_repeat.append(image_task)
            image_embeddings = torch.cat(image_embeddings_repeat, dim=0)
            image_tasks = torch.cat(image_tasks_repeat,0)

            masks, low_res_masks, iou_predictions = prompt_and_decoder(args, batched_input, model, image_embeddings,
                                                                       decoder_iter=False,image_tasks=image_tasks)
            loss = criterion(masks, labels, iou_predictions)
            loss += mag_loss
        ...
        if random.random() < 0.5:
            ...
            for iter in range(args.iter_point):
                ...
                if args.use_amp:
                    masks, low_res_masks, iou_predictions = prompt_and_decoder(args, batched_input, model, image_embeddings,
                                                                               decoder_iter=True,image_tasks=image_tasks)
                    loss = criterion(masks, labels, iou_predictions)
                    with amp.scale_loss(loss, optimizer) as scaled_loss:
...
def main(args):
    ...
    with open(os.path.join(args.work_dir,'params.json'), 'w') as json_file:
        json.dump(args_dict, json_file, indent=4)

    model = sam_model_registry[args.model_type](args)
    if args.use_pretrain:
        state_dict = torch.load(args.pretrain_pth, map_location='cpu')
        model.load_state_dict(state_dict['model'], False)
    else:
        print('*******Do not use Pretrain')
    model = model.to(args.device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    criterion = FocalDiceloss_IoULoss()
    cls_criterion = nn.CrossEntropyLoss(ignore_index=4)
    if args.lr_scheduler:
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5, 10,20,30,40], gamma=0.5)
    ...
    for epoch in range(0, args.epochs):
        ...
        train_losses, train_iter_metrics = train_one_epoch(args, model, optimizer, train_loader, epoch, criterion,cls_criterion)
        ...
        loggers.info(f"train_epoch: {epoch + 1}, lr: {lr}, Train loss: {average_loss:.4f}, metrics: {train_metrics}")


        if average_loss < best_loss:
            best_loss = average_loss
            save_path = os.path.join(args.work_dir, "models", args.run_name, 'best_train.pth')
